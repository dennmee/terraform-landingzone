...
resource "aws_subnet" "edge" {
  count = length(var.edge_subnets)

  vpc_id            = aws_vpc.this.id
  cidr_block        = element(var.edge_subnets, count.index)
  availability_zone = element(data.aws_availability_zones.available.names, count.index)
  tags = merge(
    var.common_tags,
    var.edge_subnet_tags,
    {
      "Name" = format(
        "%s-%s-%s-%s",
        ...
        lower(var.environment_name),
        lower(var.edge_subnet_tier_name),
      )
    },
  )
}
...
resource "aws_network_acl" "edge" {
  vpc_id     = aws_vpc.this.id
  subnet_ids = aws_subnet.edge.*.id

  dynamic "ingress" {
    for_each = var.edge_network_acl_ingress_rules
    content {
      rule_no    = ingress.value.rule_no
      action     = ingress.value.action
      from_port  = ingress.value.from_port
  ...
  dynamic "egress" {
    for_each = var.edge_network_acl_egress_rules
    content {
      rule_no    = egress.value.rule_no
      action     = egress.value.action
      from_port  = egress.value.from_port
